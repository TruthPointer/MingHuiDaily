package org.tpmobile.minghuidaily.widget
 ...
 *
 * Modified by tp on 20260124
 */
open class VideoEnabledWebChromeClient : WebChromeClient, OnPreparedListener, OnCompletionListener,
    ...
    private lateinit var activityVideoView: ViewGroup
    ...
    var isVideoFullscreen =
        false // Indicates if the video is being displayed using a custom view (typically full-screen) = false
    ...
    override fun onShowCustomView(view: View, callback: CustomViewCallback) {
        if (view is FrameLayout) { // A video wants to be shown
            ...
            videoViewContainer?.apply { setBackgroundColor(Color.BLACK) }
            ...
            if (focusedChild is VideoView) { // android.widget.VideoView (typically API level <11)
                ...
                webView?.let {
                    if (it.settings.javaScriptEnabled && focusedChild is SurfaceView) {// Run javascript code that detects the video end and notifies the Javascript interface
                        var js = "javascript:"
    ...
    override fun onHideCustomView() {
        println("STATE: onHideCustomView().....")
